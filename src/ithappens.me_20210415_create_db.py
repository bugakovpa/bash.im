...
c.execute("CREATE TABLE IF NOT EXISTS ithappens(_id_key INTEGER PRIMARY KEY AUTOINCREMENT, id_history INT, title VARCHAR(255), content  TEXT)")
c.execute("begin")

for id_history in range (1, 20000+2):
    ...
    ##sql = '''INSERT INTO ithappens('id_history', 'title', 'content') VALUES (id_history, title, content)'''
    
    RecordKorteg = (id_history, title, content)  ## Закинули строку по полям в кортеж.
    if DebugMode : print ("RecordKorteg:", RecordKorteg)
    
    ZaprosSQLInsert = "INSERT INTO \"ithappens\" (\"id_history\", \
    \"title\", \
