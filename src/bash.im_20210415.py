...
for id_history in range (454600, 461061+1): ## На текущий момент существует 461061 цитата.
    # Запрос в БД.
    ...
    ZaprosSQLSelect = "SELECT \"content\" FROM \"bash\" WHERE \"id_history\"=" + str(id_history) + ";"
    ##DebugMode = True 
    ...
    if KortegFromDB[0] == '-':  ## Если мы считали из базы дефолтное значение '-', то начинаем 
        ...
        if ((OchichenMessage.find('Утверждено <b>')) > -1):
            ...
            ZaprosSQLUpdate = "UPDATE \"bash\" SET \"content\" = \'" + str(content) + "\' WHERE \"id_history\"=" + str(id_history) + ";"
            ...
            # Синхронизируем БД
            ##if (id_history % 10 == 0): ## Записываем каждые 10 000 записей
